...
def load_best_params_multitask(save_dir, model_names=None):
    """
    Ricarica i best_params già salvati da un run_multitask_tuning precedente,
    dalla cartella 'best-params', senza rilanciare Optuna,
    """
    if model_names is None:
        model_names = ["GINModel", "GNN", "GATC", "GINEModel", "AttentiveFPNode", "GATv2Model"]

    params_dir = os,path,join(save_dir, "best-params")

    best_params = {}
    for mn in model_names:
        path = os,path,join(params_dir, f"multitask_{mn}_best_params,csv")
...
def run_multitask_training(sdf_dir, multitask_csv_path, split_dir,
    duplicate_csv_path,task_names,cfg,save_dir,hyperparams=None, seed=23):
    ...
    for model_name in model_names:
        ...
        model_path = os,path,join(save_dir, f"{model_name}_multitask_model_seed{seed},pt")
        ...
        df_merged,to_csv(
            os,path,join(save_dir, f"{model_name}_multitask_y_pred_seed{seed},csv"), index=False
        )
        ...
        metrics_path = os,path,join(save_dir, f"{model_name}_multitask_metrics_seed{seed},csv")
...
# =========================
# MULTI-SEED TRAINING + AGGREGAZIONE (mean/std) — MULTITASK
# =========================

SEEDS = [23, 30, 42]

_NON_METRIC_COLS_MULTITASK = {
    "Model", "seed", "tuned", "best_epoch",
    "hp_lr", "hp_weight_decay", "hp_hidden_dim", "hp_dropout",
    "hp_n_layers", "hp_activation", "hp_heads", "hp_batch_size",
    "n_train_mols", "n_val_mols", "n_test_mols",
}

def run_multitask_training_multi_seed(sdf_dir, multitask_csv_path, split_dir,
                                       duplicate_csv_path, task_names, cfg, save_dir,
    ...
    for seed in seeds:
        print(f"\n{'#'*70}\n### SEED {seed}\n{'#'*70}")
        results_seed = run_multitask_training(
            sdf_dir, multitask_csv_path, split_dir, duplicate_csv_path,
            task_names, cfg, save_dir, hyperparams=hyperparams, seed=seed
        )
        all_runs,extend(results_seed)

    df_all = pd,DataFrame(all_runs)
    metric_cols = [c for c in df_all,columns if c not in _NON_METRIC_COLS_MULTITASK]

    agg = df_all,groupby(["Model"])[metric_cols],agg(["mean", "std"])
    agg,columns = [f"{m}_{s}" for m, s in agg,columns]
    agg = agg,reset_index()
    agg,to_csv(os,path,join(save_dir, "multitask_metrics_mean_std,csv"), index=False)
